  ...
  for (int i = 0; i < 12; i++)
    {
      ...
      // Draw tick mark
      cairo_move_to (cr, (m_radius - inset) * cos (i * M_PI / 6.0), (m_radius - inset) * sin (i * M_PI / 6.0));
      cairo_line_to (cr, m_radius * cos (i * M_PI / 6.0), m_radius * sin (i * M_PI / 6.0));
      cairo_stroke (cr);
...
app_activate (GApplication *app, gpointer user_data)
{
  ...
  build = gtk_builder_new_from_resource ("/org/tutorial/tfc/tfc.ui");
...
main (int argc, char **argv)
{
  ...
  app = gtk_application_new ("org.tutorial.tfc", G_APPLICATION_HANDLES_OPEN);
