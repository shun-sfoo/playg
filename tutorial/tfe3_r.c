...
before_close (GtkWindow *win, gpointer user_data) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (!g_file_replace_contents (file, contents, strlen (contents), NULL, TRUE, G_FILE_CREATE_NONE, NULL, NULL,
                                  NULL)) {
...
app_open (GApplication *app, GFile **files, gint n_files, gchar *hint, gpointer user_data) {
  ...
  build = gtk_builder_new_from_resource ("/org/tutorial/tfe3/tfe3.ui");
...
main (int argc, char **argv) {
  ...
  app = gtk_application_new ("org.tutorial.tfe1", G_APPLICATION_HANDLES_OPEN);
